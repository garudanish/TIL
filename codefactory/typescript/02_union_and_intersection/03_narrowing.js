...
// 3. Truthiness Narrowing
let nullOrStr = Math.random() > 0.5 ? null : ["성남FC"];
if (nullOrStr) {
...
// 4. Equality Narrowing
let numOrStr2 = Math.random() > 0.5 ? 1234 : "성남FC";
let stringOrBool = Math.random() > 0.5 ? "성남FC" : true;
...
let numberOrStringOrNull = Math.random() > 0.5 ? 1234 : Math.random() ? "성남FC" : null;
...
let personOrDog = Math.random() > 0.5 ? person : dog;
...
// 6. instanceof Narrowing
let dateOrString = Math.random() > 0.5 ? new Date() : "성남FC";
...
switch (humanOrDog.type) {
    ...
    default:
        humanOrDog; // never
        const _check = humanOrDog; // 지금은 never 타입이기 때문에 할당할 수 있다.
